...
import { createProspect, getSales, convertToSale, pushToBackend, revertToProspect, getTargetStats, getManagerStats, requestDelete, rejectDelete, deleteSale } from '../../services/salesService';
...
const SalesDashboard = ({ isEmbedded = false }) => {
    ...
    const handleRevert = async (sale, e) => {
        ...
        if (reason) {
            try {
                await revertToProspect(sale._id, reason);
                fetchSales();
    ...
    const handleDelete = async (sale, e) => {
        e.stopPropagation();

        const isManager = ['Admin', 'Super Admin', 'Sales Manager'].includes(user.role);
        const actionText = isManager ? 'Delete Permanently' : 'Request Deletion';

        const { value: reason } = await Swal.fire({
            title: `${actionText}?`,
            text: `Are you sure you want to ${actionText.toLowerCase()} "${sale.clientName}"?`,
            input: 'text',
            inputLabel: 'Reason',
            inputPlaceholder: 'Enter reason...',
            inputValidator: (value) => {
        ...
        if (reason) {
            try {
                if (isManager) {
                    await deleteSale(sale._id, reason);
                    Swal.fire('Deleted!', 'Sale has been deleted.', 'success');
                } else {
                    await requestDelete(sale._id, reason);
                    Swal.fire('Requested!', 'Deletion request sent to manager.', 'success');
                }
                fetchSales();
            } catch (err) {
    ...
    const handleApproveDelete = async (sale, e) => {
        e.stopPropagation();
        const result = await Swal.fire({
            title: 'Approve Deletion?',
            text: `Confirm deletion of "${sale.clientName}" requested by ${sale.deleteRequest?.requestedBy?.name}? Reason: "${sale.deleteRequest?.reason}"`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
            try {
                await deleteSale(sale._id, sale.deleteRequest?.reason); // Pass original reason
                fetchSales();
                Swal.fire('Deleted!', 'Sale has been deleted.', 'success');
            } catch (err) {
    ...
    const handleRejectDelete = async (sale, e) => {
        e.stopPropagation();
        try {
            await rejectDelete(sale._id);
            fetchSales();
        ...
        <div className={`flex bg-white dark:bg-black overflow-hidden transition-colors duration-300 ${isEmbedded ? 'h-full' : 'h-screen'}`}>
            ...
            <div className="flex-1 overflow-auto p-6">
                <div className="max-w-full">
                    ...
                    {/* Deletion Requests (Admin/Manager Only) */}
                    {['Admin', 'Super Admin', 'Sales Manager'].includes(user.role) && sales.some(s => s.deleteRequest?.isRequested) && (
                        <div className="bg-red-50 dark:bg-red-900/10 p-6 rounded-2xl border border-red-100 dark:border-red-900/30 mb-8 animate-pulse">
                            <h3 className="font-bold text-red-600 dark:text-red-400 mb-4 flex items-center gap-2">
                                <span className="text-xl">⚠️</span> Pending Deletion Requests
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {sales.filter(s => s.deleteRequest?.isRequested).map(sale => (
                                    <div key={sale._id} className="bg-white dark:bg-black p-4 rounded-xl shadow-sm border border-red-100 dark:border-red-900/30">
                                        <div className="flex justify-between items-start mb-2">
                                            <h4 className="font-bold text-zinc-900 dark:text-zinc-100">{sale.clientName}</h4>
                                            <span className="text-[10px] font-bold text-red-500 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded">REQUESTED</span>
                                        </div>
                                        <p className="text-xs text-zinc-500 mb-1">By: {sale.deleteRequest?.requestedBy?.name}</p>
                                        <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300 italic mb-3">"{sale.deleteRequest?.reason}"</p>
                                        <div className="flex gap-2">
                                            <button onClick={(e) => handleApproveDelete(sale, e)} className="flex-1 bg-red-600 text-white py-2 rounded-lg text-xs font-bold hover:bg-red-700">Approve Delete</button>
                                            <button onClick={(e) => handleRejectDelete(sale, e)} className="flex-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 py-2 rounded-lg text-xs font-bold hover:bg-zinc-200 dark:hover:bg-zinc-700">Reject</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                ...
                                {sales.filter(s => s.status === 'Prospect').map(sale => (
                                    <div key={sale._id} onClick={() => sale.hasUnreadManagerComment ? navigate('/sales/discussions') : openDetailModal(sale)} className="relative bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 hover:shadow-xl transition-all cursor-pointer group hover:-translate-y-1 ring-1 ring-transparent hover:ring-[#D8F60D]/50">
                                        ...
                                        <div className="flex justify-between items-center mt-4">
                                            ...
                                            <button onClick={(e) => handleDelete(sale, e)} className="ml-2 text-zinc-400 hover:text-red-500 p-2">
